...
class TextProcessor:
    def __init__(self, tokenizer: AutoTokenizer, text_encoder):
        """
    ...
    def encode_prompt_and_cfg(
        ...
        # comfy clip
        tokens_p = self.text_encoder.tokenize(prompt[0])
        prompt_embeds = self.text_encoder.encode_from_tokens_scheduled(tokens_p)[0][0] 
        #print(prompt_embeds.shape) #torch.Size([1, 512, 4096])
        ...
        uncond_seq_lens = uncond_attention_mask.gt(0).sum(dim=1).long()
         ...
         # comfy clip
        tokens_p = self.text_encoder.tokenize(negative_prompt[0])
        negative_prompt_embeds = self.text_encoder.encode_from_tokens_scheduled(tokens_p)[0][0] 
        ...
        prompt_embeds = prompt_embeds.repeat(num_videos_per_prompt, 1, 1)
        negative_prompt_embeds = negative_prompt_embeds.repeat(num_videos_per_prompt, 1, 1)
        #print(prompt_embeds.shape,"after padding")
        return prompt_embeds, negative_prompt_embeds
    
    @torch.no_grad() 
    def encode_prompt_and_cfg_(
        self,
        prompt: Union[str, List[str]],
        device: Optional[torch.device] = None,
        dtype: Optional[torch.dtype] = None,

...
class MaskVAEFinetuner(nn.Module):
    ...
    """
    def __init__(self,  vae_model_id,vae_path, target_dtype=torch.bfloat16):
        super().__init__()
        print(f"Loading VAE from {vae_model_id} (subfolder 'vae')...")
        config= AutoencoderKLWan.load_config(os.path.join(vae_model_id,"vae/config.json") )
        self.vae=AutoencoderKLWan.from_config(config).to(dtype=target_dtype)
        if vae_path.endswith(".safetensors"):
            vae_dict=load_file(vae_path,device="cpu")
        else:
            vae_dict=torch.load(vae_path,map_location='cpu', weights_only=False)
